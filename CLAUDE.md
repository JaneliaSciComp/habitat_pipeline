...
- **"The session's duration" is not a thing — animals in one session share a clock, not a recording length.** On `20251216` the four durations are 18866 s (rat613), 3651 s (rat615), 18556 s (rat630) and 9960 s (rat631). So any quantity phrased as a fraction *of the recording* is per-animal: the single tracking window `[687.3, 8187.2]` covers 39.8% / 205% / 40.4% / **75.3%** of it depending on the animal, and 205% being impossible is the tell that a session-wide scalar was never coherent. Read `tracking.coverage_by_animal[animal]` and `events.frac_events_within_recording_by_animal[animal]` from the manifest, never the session-wide fields (which are kept only as the most conservative value and name their reference animal). This bit twice while building Layer 0: the probe originally divided by whichever animal sorted first and reported that as the coverage.
