...
- **"The session's duration" is not a thing — animals in one session share a clock, not a recording length, and a recording does not start at zero.** What matters is each animal's measured `[first spike, last spike]` window on the day clock (`ephys.per_animal.<animal>.ephys_window` in the manifest), *not* `[0, duration_seconds]`. On `20251216` those windows are `[6.7, 18897.5]` (rat613), `[377.0, 4028.7]` (rat615), `[6.7, 18563.3]` (rat630) and `[6.7, 18377.4]` (rat631), so the single tracking window `[687.3, 8187.2]` covers **39.7% / 91.5% / 40.4% / 40.8%**. Read `tracking.coverage_by_animal[animal]` and `events.frac_events_within_recording_by_animal[animal]` from the manifest, never the session-wide fields (kept only as the most conservative value, and they name their reference animal). Three separate errors here, all now fixed: the probe first divided by whichever animal sorted first; then it used `[0, duration]`, which put rat615's coverage at an impossible **205%** and rat631's at 75.3%; **and `duration_seconds` itself is unreliable** — for rat631 it reads 9960 s from a cached pkl whose own `spike_times_by_cell` span 18370 s (raw `spike_times` is empty in that pkl, so the stale `_duration_seconds` wins). `probe_ephys` now flags that as `duration_disagrees_with_window`. The 18866/3651/18556/**9960** list documented on 2026-08-26 is superseded by the windows above.
- **Only one cohort-7 session has scored behavioural events.** Confirmed by the first capability-manifest build (2026-08-20): the events root holds one dated directory (`20251216`); tracking exists for two sessions (`20251210`, `20251216`); cohort 5 has **no tracking directory at all** and 16 of its 20 sessions are single-animal. "47 animal/session pairs" was true of *ephys* only, and is superseded: it counted what the old resolver could address (see the recording-level bullet below), against 94 animal-recordings that actually exist. Every event-based analysis has n=1 session, so cross-day and held-out confirmation of an event-based claim are not currently possible. Consult [`discovery/capability_manifest.json`](discovery/capability_manifest.json) via `check_testable(...)` rather than assuming.
...
- **A `.rec` directory is a *day*, not a recording, and the kilosort suffix is not fixed.** `20251216_094334.rec/rat613/` holds `20251216_094334_merged.*`, `20251216_144334_merged.*` and `20251216_194334_merged.*` — morning, afternoon and evening blocks, each with its own kilosort output, DIO, LFP and timestampoffset. Until 2026-09-02 `get_kilosort_path` hard-coded `f"{session}_merged.kilosort"`, which named only the block matching the directory and was the wrong string entirely for cohort 5 (`_merge` in 21 of 24 animal directories, one with no suffix). Net effect: the "47 animal/session pairs" figure was the resolver's reach, not the dataset. Measured by the full probe of 2026-09-02: **96 animal-recordings exist, 94 have spike files, 63 load end-to-end — up from 38.** The 25 recovered ones are all newly-addressable non-primary blocks and carry 4320 of the 12227 quality cells in the dataset. Now [`_discover_recordings`](ingestion/data_paths.py) reads the stem off disk, and `session_id` may name any recording: `'20251216'` and `'20251216_094334'` still give the morning block (guaranteed by `tests/test_data_paths_recordings.py::TestNothingThatResolvedBeforeMoves`), `'20251216_144334'` gives the afternoon one. A full stamp that matches nothing raises rather than falling back; several matches with no primary among them also raise. `DataStorageManager` exposes `recording_id` / `is_primary_recording` / `recording_ids_on_date`, and the paths cache is at `_CACHE_VERSION = 2` because v1 entries hold `_merged` paths that may not exist.
- **A recording with spike files is not necessarily loadable: `load_kilosort_data` needs a `*.timestamps.dat` beside the `kilosort4/` directory.** `_read_timestamps` maps Kilosort's spike *indices* onto the day's absolute sample clock, and that ~2 GB file is simply absent for **31 of the 94 recordings that have spike files** — 23 of cohort 5's 24 (so cohort 5 remains effectively unusable despite its paths now resolving), plus every animal of cohort-7 sessions `20251209_160716` and `20251213_102418`. `verify_kilosort_path` returns True for these because it only checks `spike_times.npy`/`spike_clusters.npy`, so the failure surfaces at load time as `FileNotFoundError: No '*.timestamps.dat' found`. This is a missing-file problem on the share, not a code one.
- **Tracking and events resolve by 8-digit date, so on a multi-recording day every block is offered the same files** (`HZ-DATA-008`). Session `20251216` has one tracking file spanning 09:50–12:00 — inside the morning block; mapped through the 14:43 block's own sync it falls outside that recording entirely, and the afternoon block loads and syncs *cleanly*, so nothing errors. Attachment is decided by the manifest's full probe, which maps the window through each recording's sync: read `tracking.attachment_status` / `events.attachment_status` (`overlap_verified` / `no_overlap` / `undetermined`) and note that `available` is set to False on `no_overlap`. `DataStorageManager` can't decide it (that needs a DIO read, and path resolution stays cheap) so it only logs a warning — don't filter that out. A non-primary recording with unverified attachment is **ephys-only**.
