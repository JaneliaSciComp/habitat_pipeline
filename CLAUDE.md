...
- **Tracking↔ephys time conversion lives only in [`MultiAnimalSession.get_tracking_on_ephys_clock`](ingestion/multi_animal_session.py).** Anything else that needs both clocks must call that method. It also does px→cm via the optional `pixels_per_cm` config key (null by default → positions stay in pixels, `*_cm` params then mean pixels, single warning logged).
